...
# Crear un menú para seleccionar una ciudad
def mostrar_menu():
...
def mostrar_promedios_ciudad(ciudad_index):
    ...
    print(f"\nPromedios de temperatura para {ciudades_nombres[ciudad_index]}:")
    for j, semana in enumerate(ciudad):
        suma = 0
        for dia in semana:
            suma += dia['temp']
        promedio = suma / len(semana)  # Calcula el promedio
        print(f"  Semana {j + 1}: Promedio de temperatura: {promedio:.2f}°F")
    print()

# Programa principal
while True:
    eleccion = mostrar_menu()  # Mostrar menú y obtener selección de ciudad
    mostrar_promedios_ciudad(eleccion)  # Mostrar promedios de la ciudad seleccionada
